...
Start with the generated Floor Plan. It contains the stable Home layout and
tracked Console, not the complete live inventory. Add the live HUD only when
the current request depends on local state.
...
from the Floor Plan. Use `room list`, `site list` and `equipment catalog` as
local read-only inspections, and report the degraded orientation instead of
searching outside the Home for a replacement.
