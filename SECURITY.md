...
Endroit `0.10.0-alpha.0` is prerelease software. Report vulnerabilities through
[GitHub Security Advisories](https://github.com/thevzion/endroit/security/advisories/new).
Do not disclose credentials, private Workplace content or unpublished Equipment in a
public issue.
...
flowchart LR
  ...
  copy --> workplace["Source-owned Workplace"]
  workplace --> review["Source inspection + offline digest"]
...
- Canonical `WORKPLACE.md` and `DESK.md` sources must be non-empty regular UTF-8
...
- The tracked Workplace Console accepts a development launcher only as a regular,
  non-symlink file and never falls back to npm when that launcher is broken.
- Legacy SessionStart Bridges execute only the Workplace-selected Wake-up route, cap stdout
...
An approved runtime receives absolute Workplace, Desk and Equipment paths plus the
resolved context. It inherits the process environment and can perform anything the
...
composed Workplace does not authorize an agent to modify a Site or access a service.
...
Protect Workplace and Desk repositories according to the sensitivity of their
agentic material and Artifacts. Never commit credentials to Equipment, a Workplace,
