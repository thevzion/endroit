...
export const bootstrapAssets = ['@endroit/onboarding', '@endroit/hud', '@endroit/artifacts', '@endroit/targets', '@endroit/workspaces']

export async function createHome(destination, options = {}) {
  const target = resolve(destination)
  if (await exists(target)) throw new EndroitError('destination_exists', `Destination already exists: ${target}.`)
  await mkdir(dirname(target), { recursive: true })
  const stage = await mkdtemp(join(dirname(target), '.endroit-create-'))
  try {
...
async function bootstrapHomeWorkspace(root) {
  ...
  const assetRoot = join(root, 'assets', 'endroit', 'workspaces')
  ...
  for (const file of ['workspace.md', 'inbox.md']) {
    ...
    const content = template.replace(/\{\{([a-z_]+)\}\}/g, (_match, key) => {
      if (!(key in values)) throw new EndroitError('workspace_template_invalid', `Unknown Workspace template value ${key}.`)
...
export async function initHome(root = process.cwd(), options = {}) {
  ...
  try {
    ...
    const required = ['/.endroit/', mode === 'team' ? '/.desk/' : '/.desk/targets/', '/.DS_Store']
