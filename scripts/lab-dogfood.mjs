...
try {
  ...
  const hud = JSON.parse((await exec(process.execPath, [...consoleArgs, 'hud', 'json'], { cwd: home })).stdout)
  assert.equal(hud.kernel.source, 'development')
  const prompt = (await exec(process.execPath, [...consoleArgs, 'hud', 'prompt'], { cwd: home })).stdout
  assert.match(prompt, /^<endroit-hud /)
  ...
  assert.equal(hygiene.status, 'attention')
  assert.deepEqual(hygiene.findings.map(({ id }) => id), ['site-doctor:route-upstream-missing:lab:main'])
  ...
  assert.equal(parseDocument(await readFile(join(home, 'WORKPLACE.md'), 'utf8')).metadata.runtime, '@endroit/cli@0.10.0-alpha.0')
