...
try {
  ...
  await exec('npx', [...command, 'target', 'map', 'api'], { cwd: home, maxBuffer: 20 * 1024 * 1024 })
  ...
  assert.equal(hud.probes.targets.find((entry) => entry.id === 'api').map.current, true)
  assert.equal(hud.probes.targets.flatMap((entry) => entry.bindings).length, 3)
  ...
  assert.deepEqual(doctor.targets.flatMap((entry) => entry.bindings.map((binding) => binding.path)), await Promise.all(targets.map((path) => realpath(path))))
