...
- fail-closed Shared/Desk Git hooks backed by staged and historical witness checks;
...
The wizard previews every file, adapter, Git guard and commit before consent. It creates a
shared Workplace Root and a private Desk Root on `develop`, with no Room, Work,
Site, remote, hosting or delivery. Open the exact Mount path printed at the end.

The compiler and tests never automatically launch a provider or deliver a Site. Git hooks are
ergonomic guards, while `check --history` detects bypassed invalid commits.
Multi-Root Work is fail-closed until a situated Meeting proves its
Main → Manager → Worker(s) integration path.
