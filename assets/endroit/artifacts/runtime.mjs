...
const HELP = {
  create: {
    usage: 'endroit artifact create <kind> <id> --workspace <home/id|desk/id> [--status <status>] [--field <key=value>] [--derived-from <ref>] [--from <directory>] [--json]',
  ...
  promote: {
    usage: 'endroit artifact promote <selector> --to <workspace:home/id|target:id> [--binding <id>] [--json]',
  ...
  publish: {
    usage: 'endroit artifact publish <selector> --to <home|target> [--target <id>] [--binding <id>] [--json]',
...
async function listArtifacts(input) {
  ...
  for (const target of input.resolvedHome.home.settings?.['endroit/targets']?.targets ?? []) {
...
async function promoteArtifact(input, selector, flags) {
  ...
  const stage = await mkdtemp(join(dirname(targetPath), '.endroit-artifact-'))
  try {
