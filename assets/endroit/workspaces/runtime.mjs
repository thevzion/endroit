...
const HELP = {
  create: 'endroit workspace create <id> --scope <home|desk> [--summary <text>] [--json]',
  list: 'endroit workspace list [--json]',
  inspect: 'endroit workspace inspect <home/id|desk/id|id> [--json]',
  doctor: 'endroit workspace doctor [--json]',
}
...
async function route(input, command, rest, flags) {
  ...
  throw failure('usage', 'endroit workspace create|list|inspect|doctor', 2)
}
...
async function createWorkspace(input, id, flags) {
  ...
  const stage = await mkdtemp(join(dirname(destination), '.endroit-workspace-'))
  try {
...
function help(command) {
  if (!command) return [
    'Usage: endroit workspace <create|list|inspect|doctor>',
    '',
    ...Object.entries(HELP).map(([name, usage]) => `  ${name}  ${usage.replace(`endroit workspace ${name}`, '').trim()}`),
  ].join('\n')
