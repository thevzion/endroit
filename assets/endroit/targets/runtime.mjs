...
const TARGET_HELP = {
  list: {
    usage: 'endroit target list [--json]',
    effect: 'read-only',
    summary: 'List declared Targets, their Bindings and current repository evidence.',
  },
  discover: {
    usage: 'endroit target discover <root> [--json]',
    effect: 'read-only',
    summary: 'Discover Git repositories within a bounded directory.',
  },
  doctor: {
    usage: 'endroit target doctor [--json]',
    effect: 'read-only',
    summary: 'Check Target identities and Binding health.',
  },
  add: {
    usage: 'endroit target add <repository-or-path> [--id <id>] [--emoji <emoji>] [--summary <text>] [--when <situation>] [--tag <tag>] [--binding <id>] [--json]',
  ...
  unbind: {
    usage: 'endroit target unbind <target> [--binding <id>] [--delete] [--json]',
  ...
  remove: {
    usage: 'endroit target remove <target> [--json]',
  ...
  inspect: {
    usage: 'endroit target inspect <target> [--binding <id>] [--json]',
    effect: 'read-only',
...
async function route(input, command, args, flags) {
  ...
  throw failure('usage', 'endroit target list|discover|doctor|add|bind|clone|worktree|unbind|remove|inspect', 2)
}
...
async function addTarget(input, repository, flags) {
  ...
  const settings = structuredClone(home.settings?.['endroit/targets'] ?? {})
  settings.targets ??= []
  ...
  home.settings ??= {}
  home.settings['endroit/targets'] = settings
  await writeJsonAtomic(join(input.homeRoot, 'endroit.json'), home)
...
function declarations(input) {
  return input.resolvedHome.home.settings?.['endroit/targets']?.targets ?? []
}
...
async function readHome(input) {
  return JSON.parse(await readFile(join(input.homeRoot, 'endroit.json'), 'utf8'))
}
...
function helpFor(command) {
  if (!command) {
    return [
      'Usage: endroit target <command> [options]',
      '',
      'Commands:',
      ...Object.entries(TARGET_HELP).map(([name, entry]) => `  ${name.padEnd(8)} ${entry.summary}`),
      '',
      'Run endroit target <command> --help for command details.',
    ].join('\n')
  }
