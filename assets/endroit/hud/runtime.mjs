...
async function hud(input) {
  ...
  const targets = await scanTargets(plan.home.settings?.['endroit/targets']?.targets ?? [], deskRoot, artifacts)
  const items = {
...
function targetItem(target) {
  ...
  return {
    ...
    entrypoint: { model: 'endroit-target-map', user: 'endroit-target-map' },
...
async function scanTargets(targets, deskRoot, artifacts) {
  return Promise.all(targets.map(async (target) => {
    ...
    const maps = artifacts.items
      .filter((artifact) => artifact.kind === 'endroit/targets:target-map' && artifact.targets?.includes(target.id))
